...
// Shader da cobra: verde (#37df00 => (0.2157, 0.8745, 0.0))
const char* snakeFragmentShaderSource = "#version 330 core\n"
"out vec4 FragColor;\n"
"void main()\n"
"{\n"
"   FragColor = vec4(0.2157f, 0.8745f, 0.0f, 1.0f);\n"
"}\n\0";

// Shader da fruta: vermelho
...
void drawFruit(unsigned int vertexShader) {
    ...
    // Usa o shader da fruta (vermelho)
    ...
    // Liberamos os shaders compilados
...
void drawSnake(unsigned int vertexShader) {
    unsigned int fragmentShader;
    fragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
    // Usa o shader da cobra (verde)
    glShaderSource(fragmentShader, 1, &snakeFragmentShaderSource, NULL);
    glCompileShader(fragmentShader);
    unsigned int shaderProgram;
    shaderProgram = glCreateProgram();
    glAttachShader(shaderProgram, vertexShader);
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glClear(GL_COLOR_BUFFER_BIT);
        updateSnakePosition();
        // Para cada frame, recompila o vertex shader (os shaders usados nos draws são deletados)
        unsigned int tempVertexShader = glCreateShader(GL_VERTEX_SHADER);
        glShaderSource(tempVertexShader, 1, &vertexShaderSource, NULL);
        glCompileShader(tempVertexShader);
        drawFruit(tempVertexShader);
        tempVertexShader = glCreateShader(GL_VERTEX_SHADER);
        glShaderSource(tempVertexShader, 1, &vertexShaderSource, NULL);
        glCompileShader(tempVertexShader);
        drawSnake(tempVertexShader);
